...
function addToCache(keyword, values)
{
  ...
  oCache[keyword.toUpperCase()] = new Array();
  // add all the values to the keyword's entry in the cache
  for(i=0; i<values.length; i++)
    oCache[keyword.toUpperCase()][i] = values[i];
}
  ...
  for(i=keyword.length-2; i>=0; i--)
  {
    ...
    {            
      ...
      for(j=0;j<cacheResults.length;j++)
      {
        ...
        if(cacheResults[j].toUpperCase().indexOf(keyword.toUpperCase()) == 0)               
...
{  
  /* continue if keyword isn't null and the last pressed key wasn't up or 
     down */
  if(keyword != "" && !isKeyUpDownPressed && keyword.length>=(typeof(startSearch)=='number' ? startSearch : 3))
  {
    ...
    {   
      ...
      displayResults(keyword, oCache[keyword.toUpperCase()]);                          
    }
...
{  
  ...
  var div = "<table><tr><td style='background-color:black;font-weight:800;color:red;font-size:12px;font-family:Tahoma,helvetica,sans-serif;text-align:center;'>!!! Не нажимайте клавишу Enter !!!</td></tr>";
  // if the searched for keyword is not in the cache then add it to the cache
  //if(!oCache[keyword.toUpperCase()] && keyword.toUpperCase())
    addToCache(keyword, results_array);
  // if the array of results is empty display a message
  if(results_array.length == 0)
  {
    div += "<tr><td>Не найдены записи на <strong>" + keyword + 
           "</strong></td></tr>";
  ...
  else
  {
    ...
    suggestions = oCache[keyword.toUpperCase()].length;
    // loop through all the results and generate the HTML list of results
    for (var i=0; i<oCache[keyword.toUpperCase()].length; i++) 
    {  
      // retrieve the current function
      crtFunction = oCache[keyword.toUpperCase()][i];
      ...
      div += "<tr id='tr" + i + 
             ...
             "onmouseout='handleOnMouseOut(this);' onkeydown='return blockEnter(event);' onclick='setKeyword(\""+i+"\");'>" + 
             "<td align='left'><span id='a" + i + 
             "' name='" +crtFunction;
...
function setKeyword(i)
{
    var oKeyword = document.getElementById("keyword");
    var crtLink = document.getElementById("a" + i);
    oKeyword.value = crtLink.getAttribute("name");
    hideSuggestions();
...
function handleKeyUp(e) 
{
  ...
  {    
    ...
    if ((code < 13 && code != 8) || 
        (code >=14 && code < 32) || 
        (code >= 33 && code <= 46 && code != 38 && code != 40 && code != 37 && code != 39) || 
        (code >= 112 && code <= 123)) 
    {
...
function updateKeywordValue(oTr)
{
  ...
  var crtLink = document.getElementById("a" + oTr.id.substring(2,oTr.id.length));
  oKeyword.value = crtLink.getAttribute("name");
}
