...
{	
	...
	if ($sel!=false and mysql_num_rows($sel)>0 and $broken_instrument!=1 and $est_items==1)
	{
		...
		if ($sel_lumber==false OR mysql_num_rows($sel_lumber)==0)
		{
			...
			{
				$id = (int)$_GET['id'];
				$check = myquery("SELECT * FROM craft_build_stonemason WHERE id=".$id." AND user_id=0 AND build_id=$build_id");
				if ($check!=false AND mysql_num_rows($check)>0)
				{
					...
					if ($obval==0)
					{			
						myquery("UPDATE craft_build_stonemason SET user_id=$user_id,end_time=$end_time,reserve_user_id=0,reserve_time=0 WHERE id=$id");
						myquery("UPDATE craft_build_stonemason SET reserve_user_id=0,reserve_time=0 WHERE reserve_user_id=$user_id");
						$ustroil = 1;
					}
			...
			if (isset($_GET['brevn']) AND isset($_GET['tree']))
			{
				$br = (int)$_GET['brevn'];
				$tr = (int)$_GET['tree'];
				$check = @myquery("SELECT * FROM craft_build_stonemason WHERE state=200 AND build_id=$build_id AND brevn".$br.">0 AND (user_id=0 OR user_id=$user_id) AND id=$tr");
				if ($check!=false AND mysql_num_rows($check)>0)
				{
					$res = mysql_fetch_array(myquery("SELECT * FROM craft_resource WHERE id=$id_resource_blok"));
					$Res= new Res($res);
					$check=$Res->add_user(0,$user_id);
					if ($check == 1)
					{			
						myquery("UPDATE craft_build_stonemason SET brevn".$br."=0 WHERE id=$tr");
						myquery("insert into craft_stat (build_id, gp, res_id, dob, vip, dat, user, type) values (0, 0, $id_resource_blok, 0, 1, ".time().", $user_id, 'z')"); 
					}
					else
					{
			...
			else
			{
				...
				while ($tree=mysql_fetch_array($sel_tree))
				{
					...
					switch ($tree['nomer'])
					{
						case 1: $type_tree = 'Малый валун – опасность обвала '. obval_chance($tree).'%'; break;
						case 2: $type_tree = 'Средний валун – опасность обвала '. obval_chance($tree).'%'; break;
						case 3: $type_tree = 'Большой валун – опасность обвала '. obval_chance($tree).'%'; break;
						case 4: $type_tree = 'Огромный валун – опасность обвала '. obval_chance($tree).'%'; break;
						case 5: $type_tree = 'Гигантский валун – опасность обвала '. obval_chance($tree).'%'; break;
					}  
					echo '.&nbsp;&nbsp;'.$type_tree.' - ';
					if ($tree['state']<100)
					{
						echo '. Валун расколот на '.$tree['state'].'%';
					}
					elseif ($tree['state']<200)
					{
						echo '. Расколотый валун обработан на '.($tree['state']-100).'%';
					}
					else
					{
						if ($tree['reserve_user_id']==0 OR $tree['reserve_user_id']==$user_id)
						{
							...
							for ($i=1;$i<=$kol;$i++)
							{
								if ($tree['brevn'.$i]>0)
								{
									echo '<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Расколотый валун - <a href="?tree='.$tree['id'].'&brevn='.$i.'">[ взять каменный блок ]</a>';
								}
							}
						}
					}
					...
					elseif ($ustroil==0 AND $tree['state']<200)
					{
						...
						{
							echo '&nbsp;&nbsp;&nbsp;<a href="?klin='.$tree['id'].'">[ подбить валун железным клином ]</a>';
						}
					}
					echo '</div>'; 
				}
				echo '<meta http-equiv="refresh" content="60">';
				$check = myquery("SELECT id FROM craft_build_stonemason WHERE reserve_user_id=$user_id AND user_id=0 AND reserve_time>0 AND reserve_time<".time()."");
				if (mysql_num_rows($check)>0)
				{
					myquery("UPDATE craft_build_stonemason SET user_id=$user_id,end_time=$end_time,reserve_user_id=0,reserve_time=0 WHERE reserve_user_id=$user_id AND user_id=0");
		...
		{                                  
			...
			if ($lumber['end_time']>time())
			{
				...
				if ($lumber['state']<100)
				{                                             
					echo'Ты '.echo_sex('занят','занята').' раскалыванием валуна. (Готовность - '.$lumber['state'].'%)'; 
				}
				else
				{
					echo'Ты '.echo_sex('занят','занята').' обработкой расколотого валуна. (Готовность - '.($lumber['state']-100).'%)'; 
				}      
			...
			else
			{
				...
				add_exp_for_craft($user_id, 5);
				                   
				//таймер окончен
				$action1 = '';
				if($lumber['state']==0)
				{
					$action1 = 'Ты '.echo_sex('закончил','закончила').' раскалывание валуна на 20%';
					myquery("UPDATE craft_build_stonemason SET state=20,end_time=0,user_id=0,reserve_user_id=$user_id,reserve_time=".(time()+60)." WHERE id=".$lumber['id']."");  
					break_axe($action1);
				}
				if($lumber['state']==20)
				{
					$action1 = 'Ты '.echo_sex('закончил','закончила').' раскалывание валуна на 40%';
					myquery("UPDATE craft_build_stonemason SET state=40,end_time=0,user_id=0,reserve_user_id=$user_id,reserve_time=".(time()+60)." WHERE id=".$lumber['id']."");                          
					break_axe($action1);
				}
				if($lumber['state']==40)
				{
					$action1 = 'Ты '.echo_sex('закончил','закончила').' раскалывание валуна на 60%';					
					myquery("UPDATE craft_build_stonemason SET state=60,end_time=0,user_id=0,reserve_user_id=$user_id,reserve_time=".(time()+60)." WHERE id=".$lumber['id']."");                          
					break_axe($action1);
				}
				if($lumber['state']==60)
				{
					$action1 = 'Ты '.echo_sex('закончил','закончила').' раскалывание валуна на 80%';
					myquery("UPDATE craft_build_stonemason SET state=80,end_time=0,user_id=0,reserve_user_id=$user_id,reserve_time=".(time()+60)." WHERE id=".$lumber['id']."");                          
					break_axe($action1);
				}
				if($lumber['state']==80)
				{
					$action1 = 'Ты '.echo_sex('закончил','закончила').' раскалывание валуна на 100%';
					myquery("UPDATE craft_build_stonemason SET state=100,end_time=0,user_id=0,reserve_user_id=$user_id,reserve_time=".(time()+60).",chance=0 WHERE id=".$lumber['id'].""); 
					setCraftTimes($user_id,5,1,1);
					break_axe($action1);           
				}
				if($lumber['state']==100)
				{
					$action1 = 'Ты '.echo_sex('закончил','закончила').' обработку расколотого валуна на каменные блоки на 20%';
					myquery("UPDATE craft_build_stonemason SET state=120,end_time=0,user_id=0,reserve_user_id=$user_id,reserve_time=".(time()+60)." WHERE id=".$lumber['id']."");                          
					break_axe($action1);
				}
				if($lumber['state']==120)
				{
