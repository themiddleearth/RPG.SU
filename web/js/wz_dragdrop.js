...
var isLinkStarted = false;

function createObject(id, startX, startY, link) {
	var _obj = new Object();
	_obj.id = id;
	_obj.startX = startX;
	_obj.startY = startY;
...
function addObject(_array, _obj) {
...
{				  
	this.id = d_o;
	var elem = document.getElementById(d_o);
	this.type = elem.type;			 
	this.cmd = '';
	...
	if(this.oimg)
	{
		...
		this.t_htm = '<div id="' + this.id +
			...
			'height:' + this.h + 'px;">'+
			this.htm + '</div>'; 
//		addObject(_objects, createObject(d_o, this.x, this.y));
		addObject(_objects, createObject(d_o, findPosX(elem), findPosY(elem), elem.link));		
	}
	else
	{
...
function DRAG(d_ev)
{
     if (isLinkStarted) return;
	dd.obj.resizeTo(50, 50);
...
function isAvailableDrag(name) {   
	var elem = document.getElementById(name);
	var slot;
	if (elem.type == 'Оружие') {
	  slot = document.getElementById('Оружие');
	} else if (elem.type == 'Доспех') {
	  slot = document.getElementById('Доспех');	
	} else if (elem.type == 'Шлем') {
	  slot = document.getElementById('Шлем');	
	} else if (elem.type == 'Пояс') {
	  slot = document.getElementById('Пояс');	
	} else if (elem.type == 'Щит') {
	  slot = document.getElementById('Щит');	
	} else if (elem.type == 'Кольцо') {
	  slot = document.getElementById('Кольцо');	
	} else if (elem.type == 'Магия') {
	  slot = document.getElementById('Магия');	
	} else if (elem.type == 'Артефакт') {
	  slot = document.getElementById('Артефакт');	
	}			
//	alert(event.clientX +', ' + event.clientY);
//	alert(findPosX(slot) +', ' + findPosY(slot));
	if ((findPosX(slot) < event.clientX) && ((findPosX(slot) + 50) > event.clientX) &&
		(findPosY(slot) < event.clientY) && ((findPosY(slot) + 50) > event.clientY)) {
...
function DROP(d_ev)
{
	...
	if(dd.obj)
	{			
		if (isAvailableDrag(dd.obj.name)) {
			var img = document.getElementById(dd.obj.name);
			var _td = document.getElementById(img.type);
			dd.obj.moveTo(findPosX(_td), findPosY(_td));
//			alert('Угадали! Сейчас произойде перенаправление...');				
			if (!isLinkStarted) {
		...
		for (var i=0; i<_objects.length; i++) {
			if (_objects[i].id == dd.obj.name) {
//				var divElem = document.getElementById(dd.obj.id);
//				alert(_objects[i].startX + ', ' + _objects[i].startY);
//				divElem.style.top = _objects[i].startY + 'px';
//				divElem.style.left = _objects[i].startX + 'px';
				dd.obj.moveTo(_objects[i].startX, _objects[i].startY);
				dd.obj.resizeTo(30, 30);
			}
		}
